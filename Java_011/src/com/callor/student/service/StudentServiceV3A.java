...
public class StudentServiceV3A {
	...
	public boolean inputStudent() {
		...
		if(stdDto.name == null) 	return false;
		stdDto.dept = this.itemInput("학과");
		if(stdDto.dept == null) 	return false;
		stdDto.grade = this.itemInput("학년");
		if(stdDto.grade == null) 	return false;
		stdDto.tel = this.itemInput("전화번호");
		if(stdDto.tel == null) 	return false;
		stdDto.addr = this.itemInput("주소");
		if(stdDto.addr == null) 	return false;
	...
	public void inputStudents() {
		while(true) {
			...
			// if(bYes == false) { bYes 가 false 이냐 라는 의미
			// if(bYes != true) { bYes 가 true 이냐 라는 의미
			if(!bYes) {
				break;		
			}
		}
